...
The template repository uses the canonical proprietary notice. Generated
repositories select the company profile: R packages use GPL version 2 or later
with `inst/COPYRIGHTS`; static websites and WAEF-style frameworks use the
proprietary notice. Other project types require accountable approval before
publication.
